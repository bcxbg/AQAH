...
if __name__=='__main__': 
    ...
    total_question_dict.update(question_dict)
    # question_path = "/data/dtt/project/LLaVA-main/hallucination/code/generate_question/generate_question_1000.json"
    # with open(file = question_path,mode = 'r',encoding='utf-8') as file:
    #     question_dict = json.load(file)
    # total_question_dict.update(question_dict)
    # question_path = "/data/dtt/project/LLaVA-main/hallucination/code/generate_question/generate_question_2000.json"
    # with open(file = question_path,mode = 'r',encoding='utf-8') as file:
    #     question_dict = json.load(file)
    # total_question_dict.update(question_dict)
    rs_dict = {}
    for image_name,image_path in tqdm.tqdm(image_files.items(),colour = 'red'):
        ...
        if image_name not in total_question_dict:
            continue
        prompt = total_question_dict[image_name]
        args = type('Args', (), {
            "model":model,
    ...
    with open("/data/dtt/project/LLaVA-main/hallucination/code/question_answer/6_captions_I2.json", "w") as file:
